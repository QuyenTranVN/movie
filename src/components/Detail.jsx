...
export const Detail = () => {
  ...
  const fetchCast = async () => {
    const castdata = await fetch(
      `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${APIKEY}&language=vi-VN`,
    )
  ...
  const fetchVideo = async () => {
    const data = await fetch(
      `https://api.themoviedb.org/3/movie/${id}/videos?api_key=${APIKEY}&language=vi-VN`,
    )
  ...
  return (
    <>
      {loader ? (
        ...
        <>
          <Link to='/' className='fixed z-10 text-4xl text-black bg-white m-3 md:m-5 rounded-full'>
            <HiChevronLeft />
          </Link>
                ...
                <>
                  <>
                    ...
                    >
                      <FaPlay />
                      Watch trailer{' '}
                      {Array.from(video).filter((trail) => trail.type === 'Trailer').length > 1
                        ? index + 1
                        : ''}
                    </a>
                  </>
